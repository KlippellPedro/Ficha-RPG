...
function removerItemUI(index) {
    ...
    if (!confirm(`Tem certeza que deseja remover "${nomeItem}"?`)) {
        return;
    }
    ...
    // Remove do localStorage as chaves específicas deste item ANTES de atualizarTudo
    // (global.js apenas atualiza chaves existentes no DOM, não remove chaves órfãs)
    let dados = JSON.parse(localStorage.getItem(STORAGE_KEY)) || {};
...
function limparInventario() {
    if (!confirm("Tem certeza que deseja apagar TODOS os itens do seu inventário? Esta ação não pode ser desfeita.")) {
        return;
    }
