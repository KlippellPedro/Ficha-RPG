...
(function () {
    ...
    const ELEMENT_VISUAL = {
        ...
        vida_sub:       { cor: '#33ff77', cor2: '#99ff33' },
        morte:          { cor: '#2d0a4e', cor2: '#660066' },
        luz:            { cor: '#ffd700', cor2: '#ffffff' },
        luz_sub:        { cor: '#fff5aa', cor2: '#ffffff' },
    ...
    window.createAvatarEngine = function (key, canvas) {
        ...
        if (key === 'vida' || key === 'vida_sub') {
            const isSub = key === 'vida_sub';
            const vines = []; function growVine() { return { x: Math.random() * W, y: H, segments: [], life: 1, speed: (isSub ? 0.8 : 0.5) + Math.random(), sway: Math.random() * Math.PI * 2, color: `hsl(${120 + Math.random() * 40},${isSub ? 100 : 80}%,${(isSub ? 60 : 40) + Math.random() * 20}%)` }; }
            for (let i = 0; i < (isSub ? 12 : 8); i++) vines.push(growVine());
            for (let i = 0; i < (isSub ? 25 : 15); i++) particles.push({ x: Math.random() * W, y: Math.random() * H, vy: -(0.3 + Math.random() * 0.8), vx: (Math.random() - 0.5) * 0.4, life: Math.random(), size: (isSub ? 3 : 2) + Math.random() * 4 });
            return function (f) { ctx.clearRect(0, 0, W, H); vines.forEach(v => { const maxLen = 30 + Math.random() * 20; if (v.segments.length < maxLen) { const last = v.segments.length ? v.segments[v.segments.length - 1] : { x: v.x, y: v.y }; v.segments.push({ x: last.x + Math.sin(v.sway + v.segments.length * 0.3) * 3, y: last.y - v.speed }); v.sway += 0.05; } else v.life -= 0.005; if (v.life <= 0) Object.assign(v, growVine()); if (v.segments.length < 2) return; ctx.beginPath(); ctx.moveTo(v.segments[0].x, v.segments[0].y); v.segments.forEach(s => ctx.lineTo(s.x, s.y)); ctx.strokeStyle = v.color.replace('hsl', 'hsla').replace(')', `,${v.life * 0.7})`); ctx.lineWidth = isSub ? 2 : 1.5; ctx.stroke(); const tip = v.segments[v.segments.length - 1]; ctx.beginPath(); ctx.ellipse(tip.x, tip.y, isSub ? 4 : 3, isSub ? 7 : 5, v.sway, 0, Math.PI * 2); ctx.fillStyle = v.color.replace('hsl', 'hsla').replace(')', `,${v.life * 0.8})`); ctx.fill(); }); particles.forEach(p => { p.y += p.vy; p.x += p.vx + Math.sin(f * 0.03 + p.x * 0.02) * 0.3; p.life -= 0.007; if (p.life <= 0 || p.y < -5) { p.y = H; p.x = Math.random() * W; p.life = 0.8 + Math.random() * 0.2; } ctx.beginPath(); ctx.arc(p.x, p.y, p.size * 0.5, 0, Math.PI * 2); ctx.fillStyle = `rgba(${isSub ? '150,255,100' : '100,230,80'},${p.life * 0.5})`; ctx.fill(); }); };
        }
        if (key === 'morte') {
        ...
        if (key === 'luz_sub') {
            const rays = []; for (let i = 0; i < 18; i++) rays.push({ angle: i / 18 * Math.PI * 2, len: H * 0.5 + Math.random() * H * 0.3, speed: 0.005 + Math.random() * 0.004, alpha: 0.05 + Math.random() * 0.05 });
            for (let i = 0; i < 60; i++) particles.push({ x: W / 2, y: H / 2, angle: Math.random() * Math.PI * 2, speed: 0.8 + Math.random() * 4, life: Math.random(), size: 1.5 + Math.random() * 2.5 });
            return function (f) { ctx.clearRect(0, 0, W, H); const core = ctx.createRadialGradient(W / 2, H / 2, 0, W / 2, H / 2, W * 0.2); core.addColorStop(0, `rgba(255,255,255,${0.4 + Math.sin(f * 0.08) * 0.1})`); core.addColorStop(0.5, `rgba(255,230,100,${0.2 + Math.sin(f * 0.07) * 0.05})`); core.addColorStop(1, 'transparent'); ctx.fillStyle = core; ctx.fillRect(0, 0, W, H); rays.forEach(r => { r.angle += r.speed; ctx.save(); ctx.translate(W / 2, H / 2); ctx.rotate(r.angle); const rg = ctx.createLinearGradient(0, 0, 0, -r.len); rg.addColorStop(0, `rgba(255,255,180,${r.alpha * 1.8})`); rg.addColorStop(0.5, `rgba(255,230,50,${r.alpha * 1.2})`); rg.addColorStop(1, 'transparent'); ctx.beginPath(); ctx.moveTo(-4, 0); ctx.lineTo(4, 0); ctx.lineTo(0, -r.len); ctx.closePath(); ctx.fillStyle = rg; ctx.fill(); ctx.restore(); }); particles.forEach(p => { p.x = W / 2 + Math.cos(p.angle) * p.speed * (1 - p.life) * W * 0.5; p.y = H / 2 + Math.sin(p.angle) * p.speed * (1 - p.life) * H * 0.5; p.life -= 0.015; if (p.life <= 0) { p.angle = Math.random() * Math.PI * 2; p.speed = 0.8 + Math.random() * 4; p.life = 0.8 + Math.random() * 0.2; } ctx.beginPath(); ctx.arc(p.x, p.y, p.size, 0, Math.PI * 2); ctx.fillStyle = `rgba(255,250,200,${p.life * 0.9})`; ctx.fill(); }); };
        }
        if (key === 'escuridao') {
